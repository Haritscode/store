...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "itemFoundInCart":
      return {
        ...state,
        isItemAdded: action.payload.isItemAdded,
  ...
  useEffect(() => {
    cartItem?.map((item) => {
      if (item.id === info.data.id) {
        dispatch({ type: "itemFoundInCart", payload: {isItemAdded:true,itemQuantity:item.quantitySold} });
      }
      else{
        dispatch({ type: "itemFoundInCart", payload: {isItemAdded:false,itemQuantity:0} });
      }
    });
  }, [info.data.id]);
  useEffect(() => {
    if (
      info?.data?.storeFrontOfferDiscount > 0 &&
      info?.data?.mrp > 0 &&
      info?.data?.storeFrontOfferDiscount < info?.data?.mrp
    ) {
      setDiscount(
        (
          ((info?.data?.mrp -
            (info?.data?.sellingPrice?.toFixed(2) -
              info?.data?.storeFrontOfferDiscount?.toFixed(2))) /
            info?.data?.mrp?.toFixed(2)) *
          100
        )?.toFixed(1)
      );
    } else if (
    ...
    info?.data?.sellingPrice,
    info?.data?.mrp,
    info?.data?.storeFrontOfferDiscount,
  ]);
  useEffect(() => {
    let data = [];
    if (state.isItemAdded && state.itemQuantity > 1) {
      ...
      if (itemAvilable) {
        ...
        dispatcher(
          cartData([...list.cartData, { ...info?.data, quantitySold: 1 }])
        );
      }
    ...
    <>
      <div className="Item">
        <span
          ...
          style={discount <= 0 ? { display: "none" } : { display: "block" }}
        >
          {discount}% off
        </span>
        <img
          className="Item_img"
          src={
            info?.data?.image?.length !== 0 ? info?.data?.image : subtitutingImg
          }
          alt="none"
        ...
        <div className="Item_body">
          <p
            style={
              info?.data?.outOfStock
                ? { color: "red", fontSize: "8px" }
                : { display: "none" }
            }
          >
            * Out of Stock
          ...
          <div
            className="add_item_sec"
            style={
              info?.data?.mrp > 0 && info?.data?.mrp > info?.data?.sellingPrice
                ? { alignItems: "center" }
                : {}
            }
          >
            <div className="Item_desc">
              <span className="item_price">
                <p className="item_selling_price">
                  {info?.data?.storeFrontOfferDiscount > 0
                    ? (
                        info?.data?.sellingPrice -
                        info?.data?.storeFrontOfferDiscount
                      ...
                      : info?.data?.sellingPrice -
                        info?.data?.storeFrontOfferDiscount
                    : (info?.data?.sellingPrice).toLocaleString()?.split(".")
                        ?.length > 1
                    ? info?.data?.sellingPrice.toFixed(1)
                    : info?.data?.sellingPrice}{" "}
                  ₹/{info?.data?.uom}{" "}
                </p>
                <p
                  className="item_original_price"
                  style={
                    info?.data?.mrp > 0 &&
                    info?.data?.mrp > info?.data?.sellingPrice
                      ? {}
                      : { display: "none" }
                  }
                >
            ...
            {!state.isItemAdded ? (
              <button
                disabled={info?.data?.outOfStock}
                onClick={() => dispatch({ type: "itemAdded" })}
